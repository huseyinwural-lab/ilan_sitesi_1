...
export default function SupportApplications({ applicationType, title, subtitle, testIdPrefix }) {
  ...
  const handleStatusChange = async (applicationId, value) => {
    ...
    if (['approved', 'rejected'].includes(value)) {
      decisionReason = window.prompt('Karar gerekçesi girin');
      if (!decisionReason) {
        return;
      }
    }
    ...
    try {
      await axios.patch(
        ...
        { status: value, decision_reason: decisionReason },
        { headers: authHeader }
      );
                ...
                items.map((item) => {
                  ...
                  return (
                    <tr key={item.id} className="border-b last:border-none" data-testid={`${testIdPrefix}-row-${item.id}`}>
                      <td className="p-3" data-testid={`${testIdPrefix}-name-${item.id}`}>{item.user?.name || '-'}</td>
                      <td className="p-3" data-testid={`${testIdPrefix}-email-${item.id}`}>{item.user?.email || '-'}</td>
                      <td className="p-3" data-testid={`${testIdPrefix}-country-${item.id}`}>{item.user?.country || '-'}</td>
                      <td className="p-3" data-testid={`${testIdPrefix}-type-${item.id}`}>
                        {item.category === 'complaint' ? 'Şikayet' : 'Talep'}
                      </td>
