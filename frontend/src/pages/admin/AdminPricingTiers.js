  ...
  return (
    <div className="space-y-5" data-testid="admin-pricing-individual-page">
      ...
      <div className="rounded-lg border bg-white p-4" data-testid="admin-pricing-individual-table">
        <div className="mb-3 flex flex-wrap gap-2" data-testid="admin-pricing-individual-type-summary">
          {typeQuotaSummary.map((entry) => (
            <div
              key={entry.value}
              className="rounded-full border px-3 py-1 text-xs font-semibold"
              data-testid={`admin-pricing-individual-type-summary-${entry.value}`}
            >
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm" data-testid="admin-pricing-individual-table-grid">
            ...
            <tbody>
              {items.length === 0 && (
                <tr>
                  <td colSpan="8" className="py-6 text-center text-xs text-muted-foreground" data-testid="admin-pricing-individual-empty">
                    Henüz kampanya yok.
                  </td>
                </tr>
              )}
              {items.map((item) => (
                <tr key={item.id} className="border-t" data-testid={`admin-pricing-individual-row-${item.id}`}>
                  <td className="py-3 pr-3" data-testid={`admin-pricing-individual-type-${item.id}`}>
            ...
            <div className="space-y-3">
              <div>
                ...
                <select
                  ...
                  data-testid="admin-pricing-individual-modal-listing-type"
                >
                  {LISTING_TYPE_OPTIONS.map((option) => (
