...
```bash
curl -i -X PUT "https://header-config-1.preview.emergentagent.com/api/admin/pricing/campaign" \
...
**Enable (valid):**
```bash
curl -X PUT "https://header-config-1.preview.emergentagent.com/api/admin/pricing/campaign" \
  -H "Authorization: Bearer <TOKEN>" \
  -H "Content-Type: application/json" \
