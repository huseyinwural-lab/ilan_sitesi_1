...
from app.services.moderation_etl import (
    generate_moderation_parity_report,
    run_moderation_etl,
    ModerationEtlError,
)
...
async def _ensure_test_user_two(db):
    now_iso = datetime.now(timezone.utc).isoformat()
    user_email = "user2@platform.com"
    user_password = "User123!"
...
@api_router.post("/admin/moderation/etl")
async def run_moderation_etl_endpoint(
    request: Request,
    current_user=Depends(check_permissions(["super_admin"])),
    session: AsyncSession = Depends(get_sql_session),
):
    if os.environ.get("MODERATION_ETL_ENABLED", "").lower() != "true":
        raise HTTPException(status_code=403, detail="Moderation ETL disabled")
    if APP_ENV not in MODERATION_ETL_ALLOWED_ENVS:
        raise HTTPException(status_code=403, detail="Moderation ETL not allowed in this environment")

    existing_lock = _load_moderation_etl_lock()
    if existing_lock and existing_lock.get("status") in {"running", "completed"}:
        raise HTTPException(status_code=409, detail="Moderation ETL already executed")
    ...
    _write_moderation_etl_lock(lock_payload)

    logger = logging.getLogger("moderation_etl")
    logger.info("moderation_etl_started", extra={"actor_id": actor.get("id"), "request_id": request_id})

    await _write_audit_log_sql(
        session=session,
        action="moderation_etl_started",
        actor=actor,
    ...
    try:
        ...
        parity = await run_in_threadpool(
            generate_moderation_parity_report,
            mongo_url=mongo_url,
            mongo_db=mongo_db,
        )
        ...
        await _write_audit_log_sql(
            session=session,
            action="moderation_etl_completed",
            actor=actor,
            resource_type="moderation",
            resource_id=None,
            metadata={"request_id": request_id, "state": state, "parity": parity},
            request=request,
            country_code=None,
        )
        await session.commit()

        logger.info("moderation_etl_completed", extra={"actor_id": actor.get("id"), "request_id": request_id})
        return {
            "status": "completed",
            "state": state,
            "parity": parity,
            "report_path": parity.get("report_path"),
        }
    except ModerationEtlError as exc:
        lock_payload.update(
            {
                "status": "failed",
        ...
        await _write_audit_log_sql(
            session=session,
            action="moderation_etl_failed",
            actor=actor,
            resource_type="moderation",
            resource_id=None,
            metadata={"request_id": request_id, "error_code": exc.code, "message": str(exc)},
            request=request,
            country_code=None,
        )
        await session.commit()

        logger.exception("moderation_etl_failed", extra={"actor_id": actor.get("id"), "request_id": request_id})
        if exc.code == "MONGO_CONNECTION_FAILED":
            raise HTTPException(
                status_code=503,
                detail="Mongo bağlantısı başarısız (MONGO_URL güncel değil/erişim yok). Ops’tan güncel URL talep edin.",
            ) from exc
        if exc.code in {"MONGO_URL_MISSING", "MONGO_DB_MISSING"}:
            raise HTTPException(
                status_code=500,
                detail="Mongo yapılandırması eksik (MONGO_URL/DB_NAME).",
            ) from exc
        raise HTTPException(status_code=500, detail="Moderation ETL başarısız") from exc
    except Exception as exc:
        lock_payload.update(
            {
                "status": "failed",
                "failed_at": datetime.now(timezone.utc).isoformat(),
                "error_code": "ETL_RUNTIME_ERROR",
                "error": str(exc),
            }
        )
        _write_moderation_etl_lock(lock_payload)
        ...
        await _write_audit_log_sql(
            ...
            resource_type="moderation",
            resource_id=None,
            metadata={"request_id": request_id, "error_code": "ETL_RUNTIME_ERROR", "message": str(exc)},
            request=request,
            country_code=None,
        )
        await session.commit()
