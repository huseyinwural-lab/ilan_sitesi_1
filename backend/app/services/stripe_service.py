...
from app.models.billing import Invoice, InvoiceItem, StripeEvent
...
class StripeService:
    ...
    async def _activate_subscription_if_needed(self, invoice: Invoice):
        ...
        # P18: Process Affiliate Commission
        aff_service = AffiliateService(self.db)
        ledger_entry = await aff_service.process_commission(invoice)
        if ledger_entry:
    ...
    async def handle_webhook(self, payload: bytes, sig_header: str):
        ...
        try:
            if event.type == "checkout.session.completed":
                ...
                # Handle Subscription Checkout (Not Invoice Based)
                # P14: Coupon Redemption
                session = event.data.object
                user_id = session.client_reference_id
                coupon_code = session.metadata.get("coupon_code")
                
                if user_id and coupon_code:
                    # Finalize Redemption
                    # We need to find coupon ID
                    promo_service = PromotionService(self.db)
                    
                    try:
                        # Re-validate to get ID (and ensure still valid, though paid)
                        coupon, promo = await promo_service.validate_coupon(coupon_code, user_id)
                        
                        # Calculate amount?
                        # session.total_details.breakdown.discounts contains amount.
                        amount = 0
                        if session.total_details and session.total_details.breakdown:
                            for d in session.total_details.breakdown.discounts:
                                amount += d.amount / 100 # Cents to Unit
                        
                        # Record
                        # Order ID is session ID or subscription ID? Use Session ID as ref.
                        await promo_service.record_redemption(
                            coupon_id=str(coupon.id),
                            user_id=user_id,
                            order_id=uuid.UUID(int=0), # Null or dummy UUID if no invoice ID yet
                            amount_discounted=amount
                        )
                    except Exception as e:
                        # Log error but don't fail webhook?
                        # If we fail, Stripe retries. Double redemption prevented by Unique Constraint.
                        # So raising is safe.
                        print(f"Redemption Error: {e}")
                        raise e
                
                # P17: Premium Purchase Handling
                # Check metadata for premium purchase
                if session.metadata and session.metadata.get("type") == "premium_purchase":
                    listing_id = session.metadata.get("listing_id")
                    product_key = session.metadata.get("product_key")
                    
                    if listing_id and product_key:
                        promo_service = PromotionService(self.db)
                        # We don't have admin_id here, user self-service.
                        try:
                            await promo_service.apply_promotion(listing_id, product_key)
                        except Exception as e:
                            print(f"Premium Application Error: {e}")
                            # Raise to retry
                            raise e
                ...
                if invoice:
                    ...
                    # P16: Revocation Logic
                    await self._process_revocation(
                        invoice=invoice,
                        reason=f"Refund: {charge.id}",
                ...
                # Find invoice via charge/PI... Need to lookup.
                # Assuming we can find by charge_id if we stored it, or via PI.
                # PaymentIntent ID is usually in dispute.payment_intent
                ...
                if invoice:
                    # Log risk (P16 Policy: Dispute Created = Risk Flag, No Revoke yet)
                    # We could add a note or flag to Reward.
                    # For MVP, just log structured warning.
                    print(f"⚠️ DISPUTE RISK: Invoice {invoice.id} disputed. Reward at risk.")
                ...
                if dispute.status == 'lost':
                    ...
                    if invoice:
                        await self._process_revocation(
                            invoice=invoice,
                            reason=f"Dispute Lost: {dispute.id}"
                        )
