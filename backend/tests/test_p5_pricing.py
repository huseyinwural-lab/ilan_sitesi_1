...
async def test_waterfall_pricing_flow(db_session):
    ...
    for i in range(2):
        ...
        db_session.add(Listing(
            id=listing_id, title="Test", module="auto", country="DE", user_id=user_id, dealer_id=dealer_id, status="pending"
        ))
        await db_session.flush() # Ensure it exists for FK
        
        result = await service.calculate_listing_fee(str(dealer_id), "DE", str(listing_id))
        assert result.is_free == True
        assert result.source == "free_quota"
        assert result.charge_amount == 0
        
        await service.commit_usage(result, str(listing_id), str(dealer_id), str(user_id))
    ...
    await service.commit_usage(result, str(listing_id), str(dealer_id), str(user_id), invoice_id=str(overage_inv_id))
...
async def test_missing_config_fails(db_session):
    ...
    try:
        await service.calculate_listing_fee(str(dealer_id), "FR", str(listing_id))
